...
**배포 링크 — https://worksphere-test.vercel.app**
...
배포본을 열면 설치 없이 바로 확인할 수 있다. 데이터는 브라우저의 `localStorage`에 저장되므로 방문자마다 독립적이다.
...
| `mock/api.test.ts` | 9 | 지연 범위, 실패율, 영속, **실패 시 저장소 무변경**, 목록 순서 |
| `state/applicantsReducer.test.ts` | 25 | 낙관적 이동, **롤백**, **경쟁 상태**, 되돌리기 |
...
클론한 저장소에서 `npm install && npm test`가 그대로 통과하는지도 확인했다.
